...
export const scrapeWithFallback = async (item, scraperModules) => {
  ...
  for (const scraperName of scraperPriority) {
    ...
    try {
      result = await mod.scrape(itemSlug)
    } catch (err) {
      scrapeError = err.message
      result = { isAvailable: false, pricePence: null }
    }
    ...
    await insertScrapeLog({
      ...
      errorMessage: scrapeError ?? (result.isAvailable ? null : `${scraperName} returned unavailable for ${itemSlug}`),
    })
